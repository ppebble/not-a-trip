...
Codex를 포함한 모든 작업자는 커밋 또는 PR이 필요한 저장소 작업을 시작할 때 다음 순서를 지킨다.

1. GitHub 이슈를 먼저 생성하거나 기존 이슈 번호를 확인한다.
2. `develop`을 최신화한 뒤 실제 이슈 번호가 포함된 브랜치를 만든다.
3. 작업, 검증, 커밋을 해당 이슈 브랜치에서 수행한다.
4. 원격 이슈 브랜치로 push한다.
5. `develop` 대상 PR을 생성한다.
6. 보호 브랜치에 직접 push하지 않고 PR을 통해서만 `develop`에 머지한다.

금지:

- 실제 GitHub 이슈 없이 임의 번호로 브랜치를 만들지 않는다.
- `develop` 또는 `main` 보호 브랜치에 직접 push하지 않는다.
- 로컬 fast-forward merge만 하고 PR/원격 merge를 완료했다고 보고하지 않는다.
