...
describe('Property 6: REAL_SPOT_PHOTO_FALLBACKS 해결 로직은 올바르게 동작한다', () => {
   ...
   * 우선순위(실제 사진 → controlled fallback → null)를 항상 따르는지 검증
   */
  ...
  it('bare 업로드 파일명은 공개 /uploads 경로로 변환한다', () => {
    expect(resolveThumbnailUrl('spot-with-upload', 'photo-card.webp')).toBe(
      '/uploads/photo-card.webp'
    )
  })

  it('content cover와 dummy 이미지는 랜딩 사진으로 쓰지 않고 controlled fallback을 사용한다', () => {
    const spotId = 'REAL-SPO-001'

    expect(
      resolveThumbnailUrl(
        spotId,
        '/uploads/contents/covers/synthetic-dummy.webp'
      )
    ).toBe(REAL_SPOT_PHOTO_FALLBACKS[spotId]?.imageUrl)
  })

  it('photos[0]이 플레이스홀더이고 ID가 REAL_SPOT_PHOTO_FALLBACKS에 있으면 controlled fallback 반환', () => {
    fc.assert(
      fc.property(
  ...
  it('photoUrl이 null/undefined이고 ID가 REAL_SPOT_PHOTO_FALLBACKS에 있으면 controlled fallback 반환', () => {
    fc.assert(
      fc.property(
        arbitraryFallbackSpotId,
        fc.oneof(fc.constant(null), fc.constant(undefined)),
  ...
  it('우선순위 전체 검증: 실제 사진 → controlled fallback → null', () => {
    fc.assert(
      fc.property(
        fc.oneof(
        ...
        ({ spotId, photoUrl }) => {
          ...
          // 2순위: REAL_SPOT_PHOTO_FALLBACKS에 있으면 controlled fallback
...
describe('isPlaceholderPhoto', () => {
  ...
  it('Wikimedia URL은 외부 hotlink placeholder로 판별한다', () => {
    expect(
      isPlaceholderPhoto(
