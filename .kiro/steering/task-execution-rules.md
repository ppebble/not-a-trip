...
3. 코드 품질 확인 (푸시 전 반드시 순서대로 실행):
   ```bash
   npm run format      # Prettier 포맷 적용 (CI format:check 통과 필수)
   npm run type-check  # TypeScript 타입 오류 확인
   npm run lint        # ESLint 오류 확인
   ```
4. format 실행 후 변경된 파일이 있으면 추가 커밋:
   ```bash
   git add -A
   git commit -m "chore: 코드 포맷 적용"
   ```

> ⚠️ `npm run format` 없이 푸시하면 CI의 `format:check` 단계에서 실패한다.
...
## 🔥 Checkpoint 후 핫픽스 프로세스

Checkpoint에서 사용자가 문제를 발견하면, 이미 머지된 브랜치에는 반영할 수 없다.
이 경우 별도의 핫픽스 흐름으로 수정한다.

### 핫픽스 흐름

```
사용자 피드백 → 로컬 수정 → Issue 생성 → hotfix 브랜치 → 커밋 → 푸시 → PR → 머지
```

1. 사용자 피드백을 받으면 **로컬에서 먼저 수정하여 사용자 확인**을 받는다
2. 사용자 확인 완료 후, 수정사항을 Git 흐름에 반영한다:
   - **GitHub Issue 생성** — 제목: `[fix] Spec명 - 문제 요약`
   - **브랜치 생성** — `hotfix/{이슈번호}--{간단한-요약}`
   - **커밋 & 푸시** — 이미 수정된 코드를 커밋
   - **PR 생성 → CI 확인 → 머지** — 기존 Task 흐름과 동일
3. Checkpoint Task 상태를 `completed`로 변경

### 핵심 원칙

- **로컬 수정 먼저, Git 반영은 나중에** — 사용자가 확인한 수정만 머지한다
- 핫픽스도 반드시 Issue → 브랜치 → PR → 머지 흐름을 따른다
- 핫픽스 브랜치 prefix는 `hotfix/`를 사용한다
