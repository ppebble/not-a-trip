...
5. THE OG_Image_Generator SHALL 생성된 OG 이미지 응답에 적절한 Cache-Control 헤더(예: s-maxage=86400)를 설정하여, 엣지 네트워크에서 이미지를 캐싱하고 반복적인 크롤링 요청 시 서버리스 함수 재실행을 방지한다.
...
8. IF 특정 카테고리(예: 스팟, 커뮤니티)의 URL 수가 증가하여 단일 사이트맵 파일이 비효율적일 경우, THEN THE Sitemap_Generator SHALL 사이트맵 인덱스(Sitemap Index) 방식을 도입하거나 카테고리별 사이트맵 파일(예: `/sitemap/spots.xml`)을 동적으로 분할하여 생성한다.
...
3. THE Deployment_System SHALL 프로덕션 환경 변수(MONGODB_URI, AUTH_SECRET, OAuth 클라이언트 키 등)를 Vercel 환경 변수로 관리한다.
...
4. THE Deployment_System SHALL AUTH_URL 환경 변수를 커스텀 도메인으로 설정한다.
