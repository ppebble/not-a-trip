...
- [x] 1. `isKoreanCoordinates` 함수 구현 및 테스트
  - [x] 1.1 `src/lib/directions.ts`에 `KOREA_BOUNDARY` 상수와 `isKoreanCoordinates()` 함수 추가
    - `KOREA_BOUNDARY` 상수 정의 (lat: 33.0~38.7, lng: 124.5~132.0)
    - `isKoreanCoordinates(lat, lng): boolean` 순수 함수 export
    - _Requirements: 1.1, 1.2, 1.3, 1.4_

  - [x]* 1.2 `src/lib/__tests__/directions.test.ts`에 property-based test 작성
  ...
  - [x]* 1.3 `src/lib/__tests__/directions.test.ts`에 property-based test 작성
    - **Property 2: Google Maps URL에 좌표 포함**
    - 랜덤 좌표/이름 생성 → `generateDirectionsUrls` Google Maps URL에 좌표 포함 검증
    - **Validates: Requirements 2.2**

  - [x]* 1.4 `src/lib/__tests__/directions.test.ts`에 edge case unit test 작성
...
- [x] 2. `DirectionsButton` 컴포넌트 수정 및 테스트
  - [x] 2.1 `src/components/common/DirectionsButton.tsx`에 국내/해외 조건 분기 추가
  ...
  - [x]* 2.2 `src/components/common/__tests__/DirectionsButton.test.tsx`에 컴포넌트 동작 테스트 작성
...
- [x] 3. Checkpoint — 사용자 검증
  ...
  - 사용자에게 변경사항 검증 요청: 해외 스팟 길찾기 → Google Maps 바로 열림, 국내 스팟 → 선택 메뉴 표시
  - 문제 발생 시 사용자와 논의
