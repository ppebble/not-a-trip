...
- [x] 2. Check-in API 구현
  - [x] 2.1 인증 생성 API (POST /api/checkins)
    - 이미지 URL, spotId, 방문일, 코멘트 저장
    - 유저 통계 자동 업데이트
    - _Requirements: 1.1, 1.3_

  - [x] 2.2 인증 목록 조회 API (GET /api/checkins)
    - spotId 또는 userId로 필터링
    - 최신순/인기순 정렬
    - 페이지네이션 지원
    - _Requirements: 1.5, 5.1_

  - [x] 2.3 인증 삭제 API (DELETE /api/checkins/[id])
    - 본인 인증만 삭제 가능
    - 통계 자동 업데이트
    - _Requirements: 1.3_

- [x] 3. Badge API 구현
  - [x] 3.1 뱃지 목록 조회 API (GET /api/badges)
    - 전체 뱃지 목록 반환
    - _Requirements: 4.5_

  - [x] 3.2 유저 뱃지 조회 API (GET /api/users/[id]/badges)
    - 획득한 뱃지 목록 반환
    - _Requirements: 3.1_

  - [x] 3.3 뱃지 획득 조건 체크 로직
    - 인증 생성 시 자동 호출
    - 조건 만족 시 뱃지 자동 부여
    - _Requirements: 4.1, 4.2, 4.3_

- [x] 4. User Stats API 구현
  - [x] 4.1 유저 통계 조회 API (GET /api/users/[id]/stats)
    - 총 인증 수, 방문 스팟 수, 뱃지 수 반환
    - _Requirements: 3.3_

  - [x] 4.2 콘텐츠별 진행률 조회 API (GET /api/users/[id]/progress)
    - 작품별 인증 진행률 반환
    - _Requirements: 3.2_
...
- [x] 5. 인증 UI 컴포넌트
  - [x] 5.1 CheckInButton 컴포넌트
    - 스팟 상세 페이지에 표시되는 "순례 인증" 버튼
    - 로그인 필요 시 로그인 유도
    - _Requirements: 1.1_

  - [x] 5.2 CheckInModal 컴포넌트
    - 이미지 업로드, 방문일 선택, 코멘트 입력
    - 씬 이미지와 비교 미리보기
    - _Requirements: 1.1, 1.2, 1.4_

  - [x] 5.3 ComparisonViewer 컴포넌트
    - 슬라이더 모드: 드래그로 두 이미지 비교
    - 좌우 비교 모드: 나란히 표시
    - 모바일 터치 제스처 지원
    - _Requirements: 2.1, 2.2, 2.3_

  - [x] 5.4 CheckInGallery 컴포넌트
    - 스팟별/유저별 인증샷 갤러리
    - 무한 스크롤 또는 페이지네이션
    - _Requirements: 1.5, 5.3_

- [x] 6. 프로필 UI 컴포넌트
  - [x] 6.1 UserProfilePage 페이지
    - `src/app/profile/[id]/page.tsx` 생성
    - 유저 정보, 통계, 뱃지, 인증 갤러리 표시
    - _Requirements: 3.1, 3.3_

  - [x] 6.2 TrophyRoom 컴포넌트
    - 획득한 뱃지 그리드 표시
    - 미획득 뱃지는 흐리게 표시
    - _Requirements: 3.1_

  - [x] 6.3 BadgeCard 컴포넌트
    - 뱃지 아이콘, 이름, 설명 표시
    - 획득 여부에 따른 스타일 변경
    - 진행률 표시 (미획득 시)
    - _Requirements: 4.5_

  - [x] 6.4 ContentProgressCard 컴포넌트
    - 작품별 인증 진행률 표시
    - 프로그레스 바 UI
    - _Requirements: 3.2_

- [x] 7. 뱃지 알림 컴포넌트
  - [x] 7.1 BadgeEarnedModal 컴포넌트
    - 뱃지 획득 시 축하 모달
    - 애니메이션 효과
    - _Requirements: 4.4_
