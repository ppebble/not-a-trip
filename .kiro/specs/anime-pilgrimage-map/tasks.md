...
**⚠️ 중요: 모든 작업은 반드시 Issue 생성 후 진행**

1. **GitHub Issue 생성**
   - 작업 시작 전 반드시 GitHub에서 Issue 생성
   - Issue 템플릿 활용 (Feature, Bug, UI Improvement 등)
   - 명확한 제목과 설명 작성
   - 라벨 및 마일스톤 설정

2. **Issue 번호 확인**
   - 생성된 Issue의 번호 확인 (예: #6, #7, #8)
   - 해당 번호를 브랜치명에 사용
...
```bash
# 1. GitHub에서 Issue 생성 (예: Issue #8 생성됨)
# 2. develop 브랜치에서 새 브랜치 생성
git checkout develop
git pull origin develop
git checkout -b feat/8--leaflet-map-setup
```
...
```bash
git add .
git commit -m "feat: leaflet 지도 기본 설정 구현"
git push -u origin feat/8--leaflet-map-setup
```

**3. PR 생성 및 Issue 연결**

- GitHub에서 develop 브랜치로 PR 생성
- PR 템플릿의 "관련 이슈" 섹션에 `Closes #8` 작성
- CI 통과 후 리뷰 및 머지
- PR 머지 시 연결된 Issue 자동 close
...
1. **GitHub Issue 생성**: 작업할 내용에 대한 Issue 생성 (필수)
2. **Issue 번호 확인**: 생성된 Issue 번호 메모
3. **브랜치 생성**: `{type}/{이슈번호}--{간단한-설명}` 형식으로 생성
4. **Task 상태 업데이트**: `taskStatus` 도구로 "in_progress" 설정 (도구 사용 가능 시)
5. **요구사항 확인**: 해당 task의 Requirements 번호 확인
6. **하위 작업 순서**: 하위 task가 있으면 순서대로 진행

### 각 Task 완료 후 체크리스트

1. **코드 품질 확인**: `npm run lint`, `npm run type-check`, `npm run build`
2. **Task 상태 업데이트**: `taskStatus` 도구로 "completed" 설정 (도구 사용 가능 시)
3. **커밋 및 푸시**: 의미있는 커밋 메시지로 푸시
4. **PR 생성**: develop 브랜치로 PR 생성 (템플릿 활용)
5. **Issue 연결**: PR에서 `Closes #{이슈번호}` 작성하여 Issue 자동 close 설정
...
- **다음 작업 순서**:
  1. GitHub에서 Task 5 관련 Issue 생성 (예: "지도 컴포넌트 구현")
  2. 생성된 Issue 번호 확인 (예: #8)
  3. 브랜치 생성: `feat/8--leaflet-map-setup` (PilgrimageMap 컴포넌트 기본 구현)
  4. 500줄 이하 원칙에 따라 하위 작업으로 분할 고려
